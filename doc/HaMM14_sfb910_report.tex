...
\newcommand{\thedate}{Version 0.5 \quad \today}
...
\begin{abstract}
We discuss the new solver COLDDAE for the numerical solution of linear differential-algebraic equations 
(DDAEs) of retarded and neutral type. The implementation is mainly based on the results introduced in 
\cite{HaM14}. In the single delay case, the solver can handle noncausal systems, i.e., systems, where the 
solution at the current time does not only depend on the system at the past and current time points, 
...
resulting regular, strangeness-free DDAE. 
We implemented the three stage Radau IIA collocation method, see \cite{HaiW91}.
%
%=======================================================================================================================================
...
time variable $t$ in the coefficients of \eqref{eq7}.
...
In order to handle noncausal DDAEs with single constant delays, in \cite{HaM14}, the authors proposed the 
concept of the \emph{shift index}. This concept can be directly generalized to the case of time varying delays 
as follows.
%
...
strangeness-free DDAE \eqref{eq16}. To keep the brevity of this paper, we will not repeat the process.
However, it should be noted, as has been observed in \cite{HaM14}, that the main differences between regularizing causal DDAEs and regularizing noncausal DDAEs are:
\begin{enumerate}
...
The relative error of the numerical solution of \eqref{eq20} with $m=2$ is going up fast, since additional errors are introduced by the delayed terms, see Figure \ref{fig_Paul}. 
\end{example}
%
...
\end{equation}
%
Note that the DDAE \eqref{eq21} is reformulated from the neutral DDE $\dot{x}(t)=x(t)+\dot{x}(t-1)$ by introducing a new variable to represent $x(t-1)$.\\
...
\end{align*}
and
$x_2(t)=x_1(t-1)$. The relative error of the numerical solution of \eqref{eq20} is presented in Figure \ref{fig_Paul} and goes up, too, due to the discretization and additional errors introduced by approximating the delayed terms.
...
\end{aligned}
\end{equation}
After regularization the system becomes a pure ODE and an algebraic equation.
The function $f(t)$ is chosen such that the IVP \eqref{eq23} has the unique solution $x(t)=\phi(t)$.
The relative error of the numerical solution of the IVP \eqref{eq23} is presented in Figure \ref{fig_222}.
%
\begin{figure}[h!]
 \centering
