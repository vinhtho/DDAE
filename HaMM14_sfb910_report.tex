...
\begin{array}{ccc} 
 ...
 Y_1 & \mbox{ basis of } & \range(Z^T_1 E), \\
...
\be\label{eq12}
 \bsp{
 Y^T_1 Z_1^T E(t)\dot{x}(t) &= Y^T_1 Z_1^T A(t)x(t) + Y^T_1 Z_1^T B(t) x(t-\vtau(t)) + Y^T_1 Z_1^T f(t), \\
 Y^T_2 Z^T_2 M(:,1:n) x(t)  &= Y^T_2 Z^T_2 P(:,1:kn) x(t-\vtau(t)) + Y^T_2 Z^T_2 g,
 }
\ee
%
where $\m{Y^T_1 Z_1^T E(t) \\ Y^T_2 Z^T_2 M(:,1:n)}$ is nonsingular. 
...
\begin{enumerate}
 ...
 \item[ii)] For noncausal DDAEs the set of differential equations in the strangeness-free formulation cannot be selected from the original DDAE \eqref{eq1.1a}, but must be 
...
In this solver, we have implemented step size control and so-called long steps, i.e. the step size may become greater than the delay. 
...
\end{equation}
%
{\bf Analytical solution}\quad Note that the DDAE is equivalent to the neutral DDE $\dot{x}(t)=x(t)+\dot{x}(t-1)$ . The IVP \eqref{eq21} possesses the unique solution 
$x(t)= \m{x_1(t) \\ x_2(t)}$ given by
\begin{align*}
x_1(t)=
\bc
\begin{array}{ll}
e^t & \mbox{ for all } 0<t\leq1, \\
(t-1)e^{t-1}+e^t & \mbox{ for all } 1<t\leq2, \\
\tfrac12(t^2-2t)e^{t-2}+(t-1)e^{t-1}+e^t & \mbox{ for all } 2<t\leq3,\\
\tfrac16(t^3-3t^2-3t+9)e^{t-3}+\tfrac12 (t^2-2t)e^{t-2}+(t-1)e^{t-1}+e^t & \mbox{ for all } 3<t\leq4,
\end{array}
\ec
\end{align*}
and
...
\begin{figure}[h]
 \centering
 \includegraphics[scale=0.8]{Paul_149_and_214.pdf}
