...
for i=1:N
    ...
    for j=1:3
        ...
        if t_tau<t0
            ...
            xtau = poleval_neville_aitken(t(L)+[0;c]*h,[x0_tau,X_tau],t_tau);
        end
        
        % calculate locally regularized (i.e. strangeness-free) form at t =
        % t(i)-c(j)*h
        [E1,A1,B1,f1,A2,B2,f2] = regularize_varshift_strange_lddae(E,A,B,f,tau,t(i)+c(j)*h,options);   
