function [E_1,A_1,B_1,f_1,A_2,B_2,f_2,mu,K,Z1,Z2,U1] = regularize_shift_strange_lddae(E,A,B,f,tau,ti,options)
...
for K = 0:KMax
    ...
    for mu = 0:muMax
        NMP((1:(mu+1)*m)+K*(muMax+1)*m,(1:(mu+2)*n)+K*(muMax+2)*n) = inflateEA(E,A,ti+K*tau,mu,tolR);
        if K>0
            NMP((1:(mu+1)*m)+K*(muMax+1)*m,(1:(mu+1)*n)+(K-1)*(muMax+2)*n) = -inflateB(B,ti+K*tau,mu,tolR,m,n);
            if K<K0
                continue;
            end
        ...
        M = U1'*NMP(idx1,idx2M);
        N = -U1'*NMP(idx1,idx2N);
        P = U1(1:(mu+1)*m,:)'*inflateB(B,ti,mu,tolR,m,n);
        %
        % TODO SOME COMMENTS
        ...
        g = U1'*inflatef(f,ti,tau,K,mu,tol,m);
        f_2 = Z2'*g;
        A_1 = Z1'*N;
        f_1 = Z1'*g;
        return
...
function g = inflatef(f,ti,tau,K,mu,tol,m)
%
% builds the vector
%    _        _
%   |          |
...
for j = 0:K
    for i = 1:(mu+1)
        g(((i-1)*m+1:((i-1)+1)*m)+j*(mu+1)*m) = matrix_differential(f,ti+j*tau,i-1,tol,m,1);
    end
end